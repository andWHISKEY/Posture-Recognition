# USAGE
# python posture_rtc.py --ip 127.0.0.5 --port 8000
...
music_file = "Its_a_bad_posture_voice.mp3"   # mp3 or mid file
...
states = list(range(4)) #leg,arm,Straight,Goodposture
...
#[start]yerin
def load_state():
    file=open("hyegyo.txt","r")
    text=file.read()
    states[0],states[1],states[2],states[3]=map(str,text.split('/'))
    file.close()
...
def detect_motion():
    ...
    while True:
        ...
        print()
        if (position == 1):
            #file.write("Bad Posture - Curved Back")
            print("Bad Posture - Curved Back")
            temp.append("/Curved Back/Bad Posture")
            ...
            #file.write("Bad Posture - Reclined back")
            print("Bad Posture - Reclined back")
            temp.append("/Reclined back/Bad Posture")
            pygame.mixer.music.play()
            clock = pygame.time.Clock()
            while pygame.mixer.music.get_busy():
                clock.tick(1)
            ...
            #file.write("Correct Posture - Straight")
            print("Correct Posture - Straight")
            temp.append("/Straight/Correct Posture")
        

        file.write(' '.join(temp)) # List의 Element들을 공백을 이용해서 구분
        print(temp)
        del temp[0:]
        with lock:
...
def checkHandFold(all_peaks):
    try:
            ...
            try:
                if (all_peaks[4][0][0:2]):
                    ...
                    if (distance < (armdist + 100) and distance > (armdist - 100) ): #this value 100 is arbitary. this shall be replaced with a calculation which can adjust to different sizes of people.
                        ...
                        temp.append("Not Folding Hands")
                    else: 
                        print("Folding Hands")
                        temp.append("Folding Hands")
            except Exception as e:
                print("Folding Hands")
                temp.append("Folding Hands")
    except Exception as e:
        try:
            ...
            temp.append("Unable to detect arm joints")
...
def checkKneeling(all_peaks):
    ...
    try:
        ...
        if (leftdegrees > 60  and rightdegrees > 60): # 60 degrees is trail and error value here. We can tweak this accordingly and results will vary.
            ...
            temp.append("Both Legs are in Kneeling/")
        elif (rightdegrees > 60):
            print ("Right leg is kneeling")
            temp.append("Right leg is kneeling/")
        elif (leftdegrees > 60):
            print ("Left leg is kneeling")
            temp.append("Left leg is kneeling/")
        else:
            print ("Not kneeling")
            temp.append("Not kneeling/")
        ...
        try:
            ...
            if (degrees > 60):
                ...
                temp.append("Both Legs Kneeling/")
            else:
                print("Not Kneeling")
                temp.append("Not Kneeling/")
        except Exception as e:
            print("legs not detected")
            temp.append("legs not detected/")
